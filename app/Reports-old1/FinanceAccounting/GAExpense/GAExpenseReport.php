...
namespace App\Reports\FinanceAccounting\GAExpense;
...
class GAExpenseReport extends \koolreport\KoolReport {
    ...
    function setup()
    {
        ...
        $this->ga_expense_table();
        $this->ga_expense_excel();
    }

    function ga_expense_table() {
        $node = $this->src('sqlDataSources');
        $node->query("
            EXEC sp_boc_ga_expense @period = :cut_off, @project = :project
        ")
        ->params(array(
        ...
        ->pipe(new ColumnMeta(array(
            "GA_EXPENSE_GROUP_DASHBOARD_NAME"=>array(
                "label" => "KETERANGAN",
                "type" => "string"
            ),
        ...
        ->pipe($this->dataStore('finance_accounting_ga_expense_table'));
    }

    function ga_expense_excel() {
        $node = $this->src('sqlDataSources');
        $node->query("
            EXEC sp_boc_ga_expense @period = :cut_off, @project = :project
        ")
        ...
        ->pipe($this->dataStore('finance_accounting_ga_expense_excel'));
    }
}

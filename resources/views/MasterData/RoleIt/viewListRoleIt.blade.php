        ...
        <div class="col-md">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ url('saveAddRoleIt') }}" method="post">
                    {{ csrf_field() }}
                        <div class="col-sm-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="example1" name="example1" border-style="hidden">
                            ...
                            @endphp
                            <tbody>
                                @foreach($listRoleIt as $data)
                                    <tr>
                                        <td style="text-align: center;">{{$no++}}</td>
                                        <td style="text-align: center;">{{$data->DESC_CHAR}}</td>
                                         <td>
                                            <a href="{{URL('/viewDataRoleIt/'.$data->MD_ROLE_IT_ID_INT)}}" class="btn btn-warning">Edit</a>
                                            <a href="#" class="btn btn-danger"onclick="swalDeleteData({{$data->MD_ROLE_IT_ID_INT}})">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
